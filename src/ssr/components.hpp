...
struct SectionTabs {
    ...
    static auto render(const std::vector<SectionTab>& tabs, std::string_view target, Html& h) -> void {
        h.raw("<div class=\"section-tabs\">");
        for (auto& t : tabs) {
            h.raw("<button class=\"section-tab");
            if (t.active) h.raw(" active");
            h.raw("\"");
            if (!t.url.empty()) {
                h.raw(" hx-get=\"").raw(t.url).raw("\" hx-target=\"#").raw(target)
                 .raw("\" hx-swap=\"innerHTML\" hx-trigger=\"click\"")
                 .raw(" hx-on::before-request=\"this.parentElement.querySelectorAll('.active').forEach(function(e){e.classList.remove('active')}); this.classList.add('active')\"");
            }
